...
# Question 1
# find the words that don't contain vowels in the below List
# the vowels list
# vowels = ["a", "e", "i", "o", "u"]
list_words = ["name",  "dry", "love", "cry", "run", "crypt", "Age", "Gym",
              "End", "Myth"]
# output ['dry', 'cry', 'crypt', 'Gym', 'Myth']
...
def words(word):
    # all() function return True if all the return values from check_vowels()
    ...
    # Another example dry, check_vowels return (T, T, T) then all() will
    # return True
...
# Question 2
# Get the square root and cubic root of the following list, and round them
# to 2 digits, and store in a dictionary
...
# Question 4
# Use reduce to change a nested list into one list
nest_list = [
    [1, 2, 3],
    [44, 65, 76, 89],
    [33, 90, 10, 43, 88],
...
# First change the sublists into a sets using map() function on nest_list
# no need to use a list() with map, it can be kept as object.
...
# itersection items.
# Then use reduce() with set intersection method to find intersection
