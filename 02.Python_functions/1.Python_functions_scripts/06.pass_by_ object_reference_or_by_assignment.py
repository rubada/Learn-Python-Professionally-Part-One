# In programming languages, the variables can be passed to a function either
# by:
# 1. Pass by value.
# 2. Pass by reference.
...
# The example below may appear that Python is using pass-by value but not.
...
# That means when the function is called, the function parameters or
...
# characteristics: type, value, and identity, as we learned the identity and
...
# Also, we learned that when an equal sign is used to assign obj1 to obj2, this
...
# To wrap up the above the variable name given to an object is just a
...
# Sometimes there are variables with the same name or label but refer to
# a different object with different memory addresses.
# So, when the function is called each parameter becomes the label that is
# assigned to an object, maybe they refer to the same objects or different
# objects.

# That is why in the below example when defining a variable inside the
# function, although it has the same value as the argument or variable outside
# the function, but they are different objects, and thus changing the list
# value inside the function, using the append method will not change the values
...
# The same for immutable objects, x = 30 and x = 2 are two different objects,
...
# object with the same memory address, regardless that they have different
# labels or names, (same as obj1 = obj2 mentioned above), in this case, when
# the the variable is changed inside the function it will be changed outside
# the function, that means my_list2 will be changed.
