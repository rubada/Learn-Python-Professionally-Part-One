# Function in python are considered first class objects.
# Functions in Pyhton are objects, like lists, dictionaries, strings etc.,
# that means that functions can be used in different ways like any other
...
# 2. Functions can be passed as agrument to another function:
def even_number(x):
    if x % 2 == 0:
        return f"Number {x} is even"
...
# 3.Function can be called from another function or returned inside another
# function.
...
# 4.Function can be stored in data structures such as dictionaries, lists, sets
# tuple etc.
...
# First class objects is a part of functional programming
